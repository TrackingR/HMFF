...
from lib.models.layers.MDIM_layer import MDIMLayer
from lib.models.layers.msa import MSA
...
#         B2, N2, C2 = x_c.shape
#         qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
#         qkv2 = self.qkv(x_c).reshape(B2, N2, 3, self.num_heads, C2 // self.num_heads).permute(2, 0, 3, 1, 4)
#         q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        # self.attn_1 = Attention_1(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, x_c, return_attention=False):
        if return_attention:
            ...
            # x = self.norm3(x)
            x = x + self.drop_path(self.msa(self.norm1(x), self.norm1(x_c)))
            # x = self.norm4(x)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            # xori = x_v
            # x_v = x_v + self.drop_path(self.attn(self.norm1(x_v))) + self.drop_path(self.adap_t(self.norm1(x_i)))
            # x_i = x_i + self.drop_path(self.attn(self.norm1(x_i))) + self.drop_path(self.adap_t(self.norm1(xori)))
            # xori = x_v
            # x_v = x_v + self.drop_path(self.mlp(self.norm2(x_v))) + self.drop_path(self.adap2_t(self.norm2(x_i)))
            # x_i = x_i + self.drop_path(self.mlp(self.norm2(x_i))) + self.drop_path(self.adap2_t(self.norm2(xori)))
            # return x_v, x_i
            return x


class VisionTransformerMDIM(BaseBackbone):
    """ Vision Transformer
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 tbsi_loc=None, tbsi_drop_path=None, drop_path=0.):
        """
        Args:
        ...
        # self.cbam_layers = cbam_Layers(channel=768)
        ...
        # self.conv1d = nn.Conv2d(384, 768, kernel_size=1)
        # self.conv2d = nn.Conv2d(256, 768, kernel_size=1)
        self.tbsi_loc = tbsi_loc
        # self.tbsi_hoc = [0]
        self.tbsi_drop_path = tbsi_drop_path
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.MDIM_layers = nn.ModuleList()
        ...
        if self.tbsi_loc is not None and type(self.tbsi_loc) == list:
            for i in range(len(self.tbsi_loc)):
                self.MDIM_layers.append(MDIMLayer(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = VisionTransformerMDIM(**kwargs)
