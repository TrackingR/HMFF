...
from lib.models.layers.attn_blocks import CASTBlock_1
#from lib.utils.token_utils import token2patch, patch2token
#from lib.models.cstnet.sfm import SFM

class MDIMLayer(nn.Module):
    ...
    def forward(self, x_v, x_i, lens_z):
        ...
        fused_t = self.ca_s2t_i2f(torch.cat([fused_t, x_i[:, lens_z:, :]], dim=1), torch.cat([fused_t, x_v[:, lens_z:, :]], dim=1))[:, :lens_z, :]
        temp_x_v = self.ca_t2s_f2v(torch.cat([fused_t, x_v[:, lens_z:, :]], dim=1), torch.cat([fused_t, x_i[:, lens_z:, :]], dim=1))[:, lens_z:, :]
        fused_t = self.ca_s2t_v2f(torch.cat([fused_t, x_v[:, lens_z:, :]], dim=1), torch.cat([fused_t, x_i[:, lens_z:, :]], dim=1))[:, :lens_z, :]
        temp_x_i = self.ca_t2s_f2i(torch.cat([fused_t, x_i[:, lens_z:, :]], dim=1), torch.cat([fused_t, x_v[:, lens_z:, :]], dim=1))[:, lens_z:, :]
        x_v[:, lens_z:, :] = temp_x_v
        x_i[:, lens_z:, :] = temp_x_i
