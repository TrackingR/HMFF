...
from lib.models.layers.attn import Attention_st_1
...
class CASTBlock(nn.Module):
    ...
    def forward(self, x, x1, mask=None):
        x = x + self.drop_path(self.attn_reshape(self.norm1(x), self.norm1(x1), mask))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x
class CASTBlock_1(nn.Module):

    def __init__(self, dim, num_heads, mode, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn_reshape = Attention_st_1(dim, mode, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
